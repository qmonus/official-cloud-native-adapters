...
DesignPattern: {
	parameters: {
		...
		// not use default value but define as optional parameter
		// because "unresolved disjunction" error occurs when using default value *"NodePort"
	...
	resources: app: {
		...
		if _useIngress {
			"\(parameters.appName)Ingress": templates.k8sIngressForAws & {
				#name:      parameters.appName
				#namespace: parameters.k8sNamespace
