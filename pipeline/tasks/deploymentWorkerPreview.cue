...
#Builder: {
	...
	let _workingDir = "$(workspaces.shared.path)/pulumi/$(params.appName)-$(params.deployName)-$(params.deployStateNameWithPreview)"

	params: {
		appName: desc:    "Application Name of QmonusVS With Preview"
		deployName: desc: "Deployment Name of QmonusVS With Preview"
		...
		if _input.useGcpCred {
			secretNameServiceAccount: desc: "The secret name of GCP SA credential With Preview"
	...
	steps: [{
		...
		script: """
			...
			SIGNED_URL=`curl -X POST -fs ${VS_API_ENDPOINT}'/apis/v1/projects/$(context.taskRun.namespace)/applications/$(params.appName)/deployments/$(params.deployName)/deploy-state/$(params.deployStateNameWithPreview)/action/signed-url-to-get?taskrun_name=$(context.taskRun.name)&taskrun_uid=$(context.taskRun.uid)' | xargs`
